    ...
    <div class="card-body">
      <form action="{{ route('admin.paypal-setting.update', 1) }}" method="POST">
        ...
        <div class="form-group">
          ...
          <select name="country_name" class="form-control select2">
            ...
            @foreach (config('settings.country_list') as $country)
              <option {{ $paypalSetting->country_name === $country ? 'selected' : '' }} value="{{ $country }}">
                {{ $country }}</option>
            @endforeach
          </select>
        </div>
        <div class="form-group">
          ...
          <select name="currency_name" class="form-control select2">
            ...
            @foreach (config('settings.currency_list') as $key => $currency)
              <option {{ $paypalSetting->currency_name === $currency ? 'selected' : '' }} value="{{ $currency }}">
                {{ $key }}</option>
            @endforeach
          </select>
        </div>
        <div class="form-group">
          ...
          <input type="text" value="{{ $paypalSetting->currency_rate }}" class="form-control" name="currency_rate">
        </div>
        <div class="form-group">
          <label>Paypal Client Id</label>
          <input type="text" value="{{ $paypalSetting->client_id }}" class="form-control" name="client_id">
        </div>
        <div class="form-group">
          <label>Paypal Secret Key</label>
          <input type="text" value="{{ $paypalSetting->secret_key }}" class="form-control" name="secret_key">
        </div>
