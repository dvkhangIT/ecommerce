      ...
      <div class="row">
        <div class="col-12">
          <div class="card">
            ...
            <div class="card-body">
              <form
                action="{{ route('admin.shipping-rule.update', $shipping->id) }}"
                method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                  ...
                  <select class="form-control shipping-type" name="type">
                    <option
                      {{ $shipping->type === 'flat_cost' ? 'selected' : '' }}
                      value="flat_cost">Flat Cost</option>
                    <option {{ $shipping->type === 'min_cost' ? 'selected' : '' }}
                      value="min_cost">Minimun Order Amount</option>
                  </select>
                </div>
                <div
                ...
                <div class="form-group">
                  ...
                  <select id="inputState" class="form-control" name="status">
                    <option {{ $shipping->status === 1 ? 'selected' : '' }}
                      value="1">Active</option>
                    <option
                      {{ $shipping->status === 0 ? 'selected' : '' }}value="0">
                      Inactive</option>
                  </select>
                </div>
