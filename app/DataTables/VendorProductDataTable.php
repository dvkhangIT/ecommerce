...
class VendorProductDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $deleteBtn = '<a href="'.route('vendor.products.destroy', $query->id).'" class="btn btn-danger ml-1 delete-item"><i class="far fa-trash-alt"></i></a>';
          ...
          <input class="form-check-input change-status" type="checkbox" id="status__product_vendor" data-id="'.$query->id.'">
            ...
            ->addColumn('approved', function ($query) {
                if ($query->is_approved == 1) {
                    return '<i class="badge bg-success">Appreved</i>';
                } else {
            ...
            ->rawColumns(['action', 'thumb_image', 'status', 'type', 'approved'])
            ->setRowId('id');
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('thumb_image')->width(150),
            Column::make('name'),
